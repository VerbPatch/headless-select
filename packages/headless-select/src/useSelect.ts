...
export function useSelect(initialConfig: SelectConfig): SelectInstance {
  ...
  function setState(patch: Partial<SelectState>, change?: SelectChange, forceSync = false): void {
    ...
    if (!hasChanges && !change && !forceSync) return;
    ...
    try {
      ...
      let needsFullRefresh = nativeOptions.length !== resolvedOptions.length || (hasGroups !== nativeHasGroups);
      if (!needsFullRefresh && resolvedOptions.length > 0) {
        for (let i = 0; i < resolvedOptions.length; i++) {
          if (nativeOptions[i].value !== resolvedOptions[i].value || 
  ...
  const { call: debouncedNativeChange, cancel: cancelNativeChange } = debounce<[]>(() => {
    ...
    if (optionsChanged) {
      ...
      if (selectionChanged) {
        ...
        const change: SelectChange = added.length > 0 
          ? { type: 'select', option: currentOptions.find(o => o.value === added[0]) || null }
          : removed.length > 0
            ? { type: 'deselect', option: currentOptions.find(o => o.value === removed[0]) || null }
  ...
  // We do this AFTER native sync logic (descriptors/interceptors) is set up
