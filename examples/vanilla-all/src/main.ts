  ...
  function render() {
    ...
    if (!lastState || state.selectedValues !== lastState.selectedValues) {
      if (state.selectedValues.length === 0) {
        ...
        if (config.multiple) {
          labelEl.innerHTML = state.selectedValues.map(v => `
    ...
    const listNeedsUpdate = !lastState || 
    ...
    if (state.isOpen && listNeedsUpdate) {
      if (state.isLoading) {
        ...
        if (isVirtualized) {
           const v = calculateVirtualization(state.visibleOptions.length, ITEM_HEIGHT, CONTAINER_HEIGHT, scrollTop);
           optionsToRender = v.items.map(item => state.visibleOptions[item.index]);
           offsetY = v.offsetY;
           totalHeight = v.totalHeight;
        }

        const optionsHtml = optionsToRender.map(opt => {
          if (!opt) return '';
